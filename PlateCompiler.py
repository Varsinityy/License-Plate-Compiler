...
class MaskPainter(ctk.CTkToplevel):
    def __init__(self, master, source_path, mask_path, callback):
        ...
        self.btn_black = ctk.CTkButton(
            ctrl, 
            text=" Black (Inward)", 
            image=self.master.load_icon("paintbrush.png", size=16),
            fg_color="#1c1c21", 
            border_width=2, 
            border_color=COLORS["accent_primary"], 
            command=lambda: self.set_color("red", "black")
        )
        ...
        self.btn_white = ctk.CTkButton(
            ...
            command=lambda: self.set_color("white", "white")
        )
        self.btn_white.pack(side="left", padx=5)
        
        self.slider = ctk.CTkSlider(ctrl, from_=2, to=80, command=self.set_size)
        self.slider.set(15)
        ...
        ctk.CTkButton(
            ctrl, 
            text=" Undo", 
            image=self.master.load_icon("undo.png", size=16),
            width=80, 
        ...
        ctk.CTkButton(
            ctrl, 
            text=" Apply Mask", 
            image=self.master.load_icon("square-check-big.png", size=18),
            fg_color="#10b981", 
            command=self.apply
    ...
    def set_color(self, v_color, m_color):
...
class NormalPainter(ctk.CTkToplevel):
    def __init__(self, master, base_img, callback):
        ...
        self.btn_send_comp = ctk.CTkButton(
            ...
            image=self.master.load_icon("package-plus.png", size=18),
...
class DropZone(ctk.CTkFrame):
    def __init__(self, master, label_text, file_types, dir_key, app_ref, command=None, **kwargs):
        ...
        # Placeholder icon
        self.placeholder_icon = self.app_ref.load_icon("image.png", size=36)
    ...
    def _on_click(self, event):
        ...
        self.app_ref.save_config(silent=True)
...
class PlateMakerApp(ctk.CTk):
    def __init__(self):
        ...
        self.process_ui_queue() # Start the listener
        
        self.update_idletasks()
        self.force_taskbar_presence()
        ...
        self.toggle_help_text(self.version_var.get())

        self.after(3000, self.check_for_updates) # Checks 3 seconds after opening
        self.attributes("-alpha", 0.0)
        self.animate_open()

    def animate_close(self, alpha=1.0):
        if alpha > 0:
            alpha -= 0.1 
            self.attributes("-alpha", alpha)
            self.after(10, lambda: self.animate_close(alpha))
        else:
            self.destroy()

    def animate_open(self, alpha=0.0):
        if alpha < 1.0:
            alpha += 0.1
            self.attributes("-alpha", alpha)
            self.after(10, lambda: self.animate_open(alpha))

    def animate_minimize(self, alpha=1.0):
        if alpha > 0:
            alpha -= 0.1
            self.attributes("-alpha", alpha)
            self.after(10, lambda: self.animate_minimize(alpha))
        else:
            if windll:
                hwnd = windll.user32.GetParent(self.winfo_id())
                windll.user32.ShowWindow(hwnd, 6)
    ...
    def load_assets_safe(self):
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if key == "ps":
                        ...
                        self.ps_btn_outline_eu.configure(image=self.adobe_icons["ps"], text="") # Added
                        ...
                        self.ai_btn_outline_eu.configure(image=self.adobe_icons["ai"], text="") # Added
        ...
        try:
            outline_path = resource_path("outline.png")
        ...
        try:
            outline_eu_path = resource_path("outline eu.png")
    ...
    def setup_titlebar(self):
        ...
        min_btn = ctk.CTkButton(self.titlebar, text="—", width=45, height=40, fg_color="transparent", hover_color=COLORS["bg_card"], command=self.animate_minimize, corner_radius=0)
        min_btn.pack(side="right")
    ...
    def setup_sidebar(self):
        ...
        self.btn_dashboard = ctk.CTkButton(self.nav_frame, text=" Dashboard", image=self.load_icon("layout-dashboard.png"), command=lambda: self.show_page("dashboard"), **tab_style)
        self.btn_dashboard.pack(fill="x", padx=15, pady=3)

        self.btn_generator = ctk.CTkButton(self.nav_frame, text=" Compiler", image=self.load_icon("package-plus.png"), command=lambda: self.show_page("compiler"), **tab_style)
        self.btn_generator.pack(fill="x", padx=15, pady=3)
        
        self.btn_templates = ctk.CTkButton(self.nav_frame, text=" Plate Templates", image=self.load_icon("book-dashed.png"), command=lambda: self.show_page("templates"), **tab_style)
        self.btn_templates.pack(fill="x", padx=15, pady=3)

        self.btn_editor = ctk.CTkButton(self.nav_frame, text=" Plate Designer", image=self.load_icon("square-pen.png"), command=lambda: self.show_page("editor"), **tab_style)
        self.btn_editor.pack(fill="x", padx=15, pady=3)

        self.btn_map_maker = ctk.CTkButton(self.nav_frame, text=" 3D Map Maker", image=self.load_icon("map.png"), command=lambda: self.show_page("map_maker"), **tab_style)
        self.btn_map_maker.pack(fill="x", padx=15, pady=3)

        self.btn_history = ctk.CTkButton(self.nav_frame, text=" History", image=self.load_icon("history.png"), command=lambda: self.show_page("history"), **tab_style)
        self.btn_history.pack(fill="x", padx=15, pady=3)

        self.btn_presets = ctk.CTkButton(self.nav_frame, text=" Presets", image=self.load_icon("star.png"), command=lambda: self.show_page("presets"), **tab_style)
        self.btn_presets.pack(fill="x", padx=15, pady=3)

        self.btn_settings = ctk.CTkButton(self.nav_frame, text=" Settings", image=self.load_icon("settings.png"), command=lambda: self.show_page("settings"), **tab_style)
        ...
        self.is_online = True
        self.animate_status_dot() # Start the animation
        ...
        self.btn_update = ctk.CTkButton(
            ...
            image=self.load_icon("rss.png", size=14),
    ...
    def show_page(self, page_name):
        ...
        if page_name == "dashboard":
            ...
            self.after(250, self.refresh_history) 
        ...
        self.animate_indicator(target_btn)
        self.animate_transition(getattr(self, "current_frame", None), target_frame, direction)

        self.current_page_name = page_name
        self.current_frame = target_frame

    def animate_indicator(self, target_widget, start_time=None, start_y=None, target_y=None):
        duration = 0.25 
        
        if start_time is None:
            self.nav_frame.update_idletasks() 
            if target_widget.winfo_y() <= 10:
                self.after(20, lambda: self.animate_indicator(target_widget, start_time, start_y, target_y))
                return
        ...
        if progress < 1.0:
            self.after(5, lambda: self.animate_indicator(target_widget, start_time, start_y, target_y))

    def animate_transition(self, old_frame, new_frame, direction=1, start_time=None):
        ...
        if progress < 1.0:
            self.after(5, lambda: self.animate_transition(old_frame, new_frame, direction, start_time))
        else:
            if old_frame:
                old_frame.place_forget()
            new_frame.place(relx=0.0, rely=0.0, relwidth=1.0, relheight=1.0)
    ...
    def setup_generator_page(self):
        ...
        self.version_selector = ctk.CTkSegmentedButton(
            ...
            command=lambda v: (self.toggle_help_text(v), self.save_config(silent=True))
        )
        ...
        self.region_selector = ctk.CTkSegmentedButton(
            ...
            font=ctk.CTkFont(size=12, weight="bold"), height=32,
            command=self.update_dropzone_regions
        )
        ...
        self.update_dropzone_regions(self.region_var.get())
        ...
        self.preset_data = [
            {
                "name": "Quiet Plate 1", 
                "region": "US & MX", 
                "img": resource_path("quiet plate 1 diff.png"),
                "nrml": resource_path("quiet plate 1 nrml.png")
            },
            {
                "name": "Quiet Plate 2", 
                "region": "US & MX", 
                "img": resource_path("quiet plate 2 diff.png"), 
                "nrml": resource_path("quiet plate 2 nrml.png")
            },
            {
                "name": "Japanese Plate 1", 
                "region": "US & MX", 
                "img": resource_path("japanese plate 1 diff.png"), 
                "nrml": resource_path("japanese plate 1 nrml.png")
            },
            {
                "name": "Texas Plate White", 
                "region": "US & MX", 
                "img": resource_path("texas plate white.png"), 
                "nrml": resource_path("texas white nrml.png")
            },
            {
                "name": "Texas Plate Black", 
                "region": "US & MX", 
                "img": resource_path("texas plate black.png"), 
                "nrml": resource_path("texas white nrml.png")
            }
        ]
        ...
        self.gen_dir_entry.bind("<Button-1>", lambda e: self._browse_gen_output_dir())
        ...
        self.btn_generate = ctk.CTkButton(
            self.generator_page, 
            text=" COMPILE PLATES", 
            image=self.load_icon("package-plus.png", size=24),
            fg_color=COLORS["accent_primary"], 
            height=60,
            width=0,
        ...
        self.btn_restore = ctk.CTkButton(
            ...
            image=self.load_icon("undo.png", size=18),
            fg_color=COLORS["bg_card"], 
            hover_color=COLORS["accent_danger"], 
            height=40, 
            width=0,
    ...
    def setup_map_maker_page(self):
        ...
        self.adv_switch = ctk.CTkSwitch(
            ...
            command=self.toggle_mm_advanced, 
        ...
        ctk.CTkLabel(
            ...
            image=self.load_icon("lightbulb.png", size=16), 
        ...
        self.mm_drop_zone = DropZone(self.mm_drop_container, "Source Image", [("Images", "*.png *.jpg *.jpeg")], "mm_source", self, command=self._load_preview_image)
        self.mm_drop_zone.grid(row=0, column=0, sticky="nsew", padx=2, columnspan=2)

        self.mm_mask_drop_zone = DropZone(self.mm_drop_container, "B&W Mask", [("Images", "*.png *.jpg *.jpeg")], "nrml", self, command=self._schedule_preview_update)
        ...
        self.ps_btn_preview = ctk.CTkButton(self.preview_adobe_bar, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_preview_in_adobe("photoshop"))
        self.ps_btn_preview.pack(side="right", padx=2)

        self.ai_btn_preview = ctk.CTkButton(self.preview_adobe_bar, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_preview_in_adobe("illustrator"))
        ...
        self.mm_tab_toggle = ctk.CTkSegmentedButton(
            ...
            selected_color=COLORS["accent_primary"], command=self.switch_mm_tabs
        )
        ...
        ctk.CTkSegmentedButton(self.base_slider_frame, values=["Inward", "Outward"], variable=self.base_extrude, fg_color=COLORS["bg_primary"], selected_color=COLORS["accent_primary"], command=self._schedule_preview_update).pack(fill="x", padx=20, pady=(5, 10))
        
        ctk.CTkLabel(self.base_slider_frame, text="Intensity (Depth)", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=20)
        self.base_intensity = ctk.CTkSlider(self.base_slider_frame, from_=0.1, to=10.0, button_color=COLORS["accent_primary"], command=self._schedule_preview_update)
        ...
        self.base_blur = ctk.CTkSlider(self.base_slider_frame, from_=0.0, to=5.0, button_color=COLORS["accent_primary"], command=self._schedule_preview_update)
        ...
        ctk.CTkSegmentedButton(self.mask_slider_frame, values=["Inward", "Outward"], variable=self.mask_extrude, fg_color=COLORS["bg_primary"], selected_color=COLORS["accent_primary"], command=self._schedule_preview_update).pack(fill="x", padx=20, pady=(5, 10))
        
        ctk.CTkLabel(self.mask_slider_frame, text="Intensity (Depth)", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=20)
        self.mask_intensity = ctk.CTkSlider(self.mask_slider_frame, from_=0.1, to=10.0, button_color=COLORS["accent_primary"], command=self._schedule_preview_update)
        ...
        self.mask_blur = ctk.CTkSlider(self.mask_slider_frame, from_=0.0, to=5.0, button_color=COLORS["accent_primary"], command=self._schedule_preview_update)
        ...
        self.btn_generate_map = ctk.CTkButton(
            export_btn_frame, 
            text=" EXPORT MAP", 
            image=self.load_icon("download.png", size=20),
            fg_color=COLORS["accent_secondary"], 
            height=50, 
            font=ctk.CTkFont(size=14, weight="bold"), 
            command=self.run_normal_map_gen
        )
        ...
        self.btn_paint_map = ctk.CTkButton(
            export_btn_frame, 
            text=" PAINT MAP", 
            image=self.load_icon("paintbrush.png", size=20),
            fg_color=COLORS["accent_secondary"],
            height=50, 
            font=ctk.CTkFont(size=14, weight="bold"), 
            command=self.open_normal_painter
        )
        ...
        self.btn_send_to_compiler = ctk.CTkButton(
            self.map_maker_page, 
            text=" SEND TO COMPILER", 
            image=self.load_icon("package-plus.png", size=20), 
            fg_color=COLORS["accent_primary"], 
            height=50, 
            font=ctk.CTkFont(size=14, weight="bold"), 
            command=self._send_map_to_compiler
        )
    ...
    def _update_preview(self):
        ...
        mask_path = self.mm_mask_drop_zone.get_path() if self.advanced_mode_var.get() else None
        threading.Thread(target=self._generate_preview_thread, args=(
    ...
    def _generate_preview_thread(self, b_str, b_blur, b_dir, m_str, m_blur, m_dir, mask_path):
        base_map = self._create_normal_map_data(self.mm_preview_thumb, b_str, b_blur, b_dir)
        
        if mask_path and os.path.exists(mask_path):
            try:
                mask_img = Image.open(mask_path).convert('L').resize(base_map.size)
                mask_map = self._create_normal_map_data(self.mm_preview_thumb, m_str, m_blur, m_dir)
    ...
    def run_normal_map_gen(self):
        ...
        threading.Thread(target=self._process_normal_map, args=(
            img_path, mask_path, 
            self.base_intensity.get(), self.base_blur.get(), self.base_extrude.get(),
    ...
    # Export and paint map buttons
        ...
        self.btn_generate_map = ctk.CTkButton(export_btn_frame, text="EXPORT", fg_color=COLORS["accent_secondary"], height=50, font=ctk.CTkFont(size=14, weight="bold"), command=self.run_normal_map_gen)
        self.btn_generate_map.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.btn_paint_map = ctk.CTkButton(export_btn_frame, text="PAINT MAP", fg_color=COLORS["accent_primary"], height=50, font=ctk.CTkFont(size=14, weight="bold"), command=self.open_normal_painter)
        self.btn_paint_map.pack(side="left", fill="x", expand=True, padx=(5, 0))

    def _process_normal_map(self, img_path, mask_path, b_str, b_blur, b_dir, m_str, m_blur, m_dir, save_dir, filename):
        try:
            img = Image.open(img_path)
            base_map = self._create_normal_map_data(img, b_str, b_blur, b_dir)
            
            if mask_path and os.path.exists(mask_path):
                mask_img = Image.open(mask_path).convert('L').resize(base_map.size)
                mask_map = self._create_normal_map_data(img, m_str, m_blur, m_dir)
            ...
            self.after(0, lambda: self._on_export_complete(True, out))
        except Exception as e: 
            self.after(0, lambda: self._on_export_complete(False, str(e)))
    ...
    def setup_templates_page(self):
        ...
        ctk.CTkButton(self.eu_card, text="Download", fg_color=COLORS["accent_primary"], command=lambda: self.download_template("eu")).pack(pady=20, padx=20, fill="x")
        ...
        ctk.CTkButton(self.us_card, text="Download", fg_color=COLORS["accent_primary"], command=lambda: self.download_template("us")).pack(pady=20, padx=20, fill="x")
        ...
        self.ps_btn_outline_eu = ctk.CTkButton(self.outline_eu_card, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_template("outline_eu", "photoshop"))
        self.ps_btn_outline_eu.place(relx=0.96, rely=0.04, anchor="ne")

        self.ai_btn_outline_eu = ctk.CTkButton(self.outline_eu_card, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_template("outline_eu", "illustrator"))
        ...
        ctk.CTkButton(self.outline_eu_card, text="Download", fg_color=COLORS["accent_primary"], command=lambda: self.download_template("outline_eu")).pack(pady=20, padx=20, fill="x")
        ...
        self.ps_btn_outline = ctk.CTkButton(self.outline_card, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_template("outline", "photoshop"))
        self.ps_btn_outline.place(relx=0.96, rely=0.04, anchor="ne")

        self.ai_btn_outline = ctk.CTkButton(self.outline_card, text="", width=30, height=30, fg_color="transparent", hover_color=COLORS["bg_card"], command=lambda: self.launch_template("outline", "illustrator"))
        ...
        ctk.CTkButton(self.outline_card, text="Download", fg_color=COLORS["accent_primary"], command=lambda: self.download_template("outline")).pack(pady=20, padx=20, fill="x")
    ...
    def setup_settings_page(self):
        ...
        self.create_path_setting(path_frame, "Photoshop EXE Path:", self.ps_path_var, mode="exe")
        ...
        self.create_path_setting(path_frame, "Illustrator EXE Path:", self.ai_path_var, mode="exe")
        
        # Check for bundled 7za.exe first, fall back to standard path if missing
        bundled_7z = resource_path("7za.exe")
        ...
        self.create_path_setting(path_frame, "7-Zip EXE Path: (Now built in)", self.sz_path_var, mode="exe")
        ...
        self.create_path_setting(comp_frame, "Default Output - Latest (_library Textures.zip):", self.default_out_latest_var, mode="zip")
        
        self.default_out_var = ctk.StringVar(value="C:\Games\Forza Horizon 5\media\Stripped\MediaOverride\RC0\Cars\_library")
        self.create_path_setting(comp_frame, "Default Output - v1.634 (_library Folder):", self.default_out_var, mode="dir")
        ...
        self.settings_backup_switch = ctk.CTkSwitch(
            ...
            command=lambda: self.save_config(silent=True)
        )
        ...
        ctk.CTkButton(bottom_row, text=" Clear Plate History", image=self.load_icon("trash.png"), command=self.clear_history, fg_color=COLORS["bg_card"], hover_color=COLORS["accent_danger"], height=40).pack(side="left")
        ctk.CTkButton(bottom_row, text=" Clear Zip Backups", image=self.load_icon("brush-cleaning.png"), command=self.prompt_clear_backups, fg_color=COLORS["bg_card"], hover_color=COLORS["accent_danger"], height=40).pack(side="left", padx=(10, 0))
        ctk.CTkButton(bottom_row, text=" Save Settings", image=self.load_icon("save.png"), command=self.save_config, fg_color=COLORS["accent_success"], height=40).pack(side="right")

    def create_path_setting(self, master, label, variable, mode="exe"):
        ...
        ctk.CTkButton(row, text="Browse", width=80, command=lambda: self.browse_path(variable, mode)).pack(side="right")
    ...
    def _execute_download(self, d, t):
        try:
            if t == "outline":
                ...
                src = resource_path("outline eu.png")
                if os.path.exists(src):
    ...
    def run_generation(self):
        ...
        self.history.append({
            ...
            "img": self.image_drop_zone.get_path(), 
            "nrml": self.nrml_drop_zone.get_path()
        })
    ...
    def _process_files(self, img_path, nrml_path, out_dir, silent=False):
        try:
            output_base = out_dir
            ...
            if is_latest:
                ...
                if img_path and os.path.isfile(img_path): self._generate_swatches(img_path, target_files, False, swatches_dir)
                if nrml_path and os.path.isfile(nrml_path): self._generate_swatches(nrml_path, target_files, True, swatches_dir)
    ...
    def run_restore(self):
        ...
        threading.Thread(target=self._process_restore, args=(output_base,), daemon=True).start()

    def _process_restore(self, output_base):
        try:
    ...
    def animate_button(self):
        ...
        self.after(100, self.animate_button)

    def toggle_help_text(self, value):
    ...
    def setup_history_page(self):
        ...
        ctk.CTkOptionMenu(
            ...
            corner_radius=6,
            command=lambda _: self.save_config(silent=True)
        ...
        self.history_dir_entry.bind("<Button-1>", lambda e: self._browse_gen_output_dir())
        
        ctk.CTkButton(settings_frame, text="Browse", width=80, fg_color=COLORS["bg_secondary"], hover_color=COLORS["border"], command=self._browse_gen_output_dir).pack(side="left")
    ...
    def refresh_history(self):
        ...
        for item in reversed(self.history):
            ...
            if img_path and os.path.exists(img_path):
                threading.Thread(target=self._load_history_thumbnail, args=(img_path, img_label), daemon=True).start()
            else:
                # Fallback icon
                img_label.configure(image=self.load_icon("image.png", size=24), text="")
            ...
            ctk.CTkButton(
                card, 
                text="", 
                image=self.load_icon("trash.png", size=18),
                width=30, 
                height=30, 
                fg_color="transparent", 
                hover_color=COLORS["accent_danger"], 
                command=lambda i=item: self.delete_history_item(i)
            ...
            ctk.CTkButton(
                ...
                state=btn_state, 
                fg_color=btn_color, 
                command=lambda i=item: self.toggle_cart(i)
            ).pack(side="right", padx=10)

    def _load_history_thumbnail(self, path, label):
        try:
            ...
            # Fallback icon
            self.after(0, lambda: label.configure(image=self.load_icon("image.png", size=24), text=""))

    def toggle_cart(self, item):
        region_key = 'us' if item['region'] == "US & MX" else 'eu'
        ...
        def process():
            try:
                if self.cart['eu']: 
                    ...
                    self._process_files(self.cart['eu']['img'], self.cart['eu']['nrml'], out_dir, silent=True)
                if self.cart['us']: 
                    self.region_var.set("US & MX")
                    self._process_files(self.cart['us']['img'], self.cart['us']['nrml'], out_dir, silent=True)
    ...
    def update_dropzone_regions(self, *args):
    ...
    def switch_mm_tabs(self, tab_name):
        if tab_name == "Black":
    ...
    def toggle_mm_advanced(self):
        if self.advanced_mode_var.get():
            ...
            if not hasattr(self, 'mask_btns_frame'):
                ...
                self.btn_draw_mask = ctk.CTkButton(self.mask_btns_frame, text="🖌️ New Mask", fg_color=COLORS["bg_card"], hover_color=COLORS["border"], command=lambda: self.open_mask_painter(edit=False))
                self.btn_draw_mask.pack(side="left", fill="x", expand=True, padx=(0, 5))

                self.btn_edit_mask = ctk.CTkButton(self.mask_btns_frame, text="✏️ Edit Mask", fg_color=COLORS["bg_card"], hover_color=COLORS["border"], command=lambda: self.open_mask_painter(edit=True))
            ...
            self.switch_mm_tabs("Base Settings")
            
        self._schedule_preview_update()

    def launch_preview_in_adobe(self, tool):
        ...
        def _task():
            try:
                ...
                base_map = self._create_normal_map_data(source_img, b_str, b_blur, b_dir)
                
                if mask_path and os.path.exists(mask_path):
                    mask_img = Image.open(mask_path).convert('L').resize(base_map.size)
                    mask_map = self._create_normal_map_data(source_img, m_str, m_blur, m_dir)
                    final_img = Image.composite(mask_map, base_map, mask_img)
                else:
                    final_img = base_map
    ...
    def check_for_updates(self, manual=False):
        ...
        def _task():
            try:
                ...
                if response.status_code == 200:
                    self.after(0, lambda: _set_status(True))
                    ...
                    if latest_tuple > current_tuple:
                        def prompt_update():
                            ...
                            def on_yes():
                                ...
                                if exe_url:
                                    threading.Thread(target=self._execute_auto_update, args=(exe_url,), daemon=True).start()
                                else:
                            ...
                            ctk.CTkButton(btn_frame, text="Not Now", width=120, fg_color=COLORS["bg_card"], hover_color=COLORS["border"], command=on_no).pack(side="left", expand=True, padx=(20, 10))
                            ctk.CTkButton(btn_frame, text="Install Update", width=120, fg_color=COLORS["accent_success"], hover_color="#059669", command=on_yes).pack(side="right", expand=True, padx=(10, 20))
                    ...
                    self.after(0, lambda: _set_status(False)) # Set Offline on bad status code
                    if manual:
                        self.after(0, lambda: messagebox.showerror("Update Error", "Could not connect to GitHub."))
            except Exception as e:
                self.after(0, lambda: _set_status(False)) # Set Offline on crash/no internet
                if manual: self.after(0, lambda err=e: messagebox.showerror("Update Error", f"An error occurred: {err}"))

        import threading
        threading.Thread(target=_task, daemon=True).start()

    def _execute_auto_update(self, download_url):
        try:
...
"""
    ...
    def setup_editor_page(self):
        ...
        self.state_dropdown = ctk.CTkOptionMenu(controls_frame, variable=self.state_var, values=list(PLATE_TEMPLATES.keys()), command=self._on_state_change)
        ...
        self.plate_text_var.trace_add("write", self._on_text_change)
        ...
        self.tags_switch = ctk.CTkSwitch(
            ...
            command=self._update_editor_preview, 
            button_color=COLORS["accent_primary"]
        )
        ...
        self.btn_save_custom = ctk.CTkButton(
            ...
            width=1200,
            font=ctk.CTkFont(size=14, weight="bold"),
            command=self._save_custom_plate
        )
        ...
        self.btn_send_to_mm = ctk.CTkButton(
            ...
            image=self.load_icon("map.png", size=20), 
            fg_color=COLORS["accent_primary"], 
            height=50, 
            width=1200,
            font=ctk.CTkFont(size=14, weight="bold"),
            command=self._send_to_map_maker
        )
        self.btn_send_to_mm.pack(fill="x", padx=0, pady=(0, 20))

        # Initial render to check default state toggles
        self._on_state_change(self.state_var.get())

    def _on_state_change(self, choice):
    ...
    def _on_text_change(self, *args):
        ...
        self._render_job = self.after(300, self._update_editor_preview)
    ...
    def _generate_plate_image(self):
        ...
        # Wrap the image path
        image_path = resource_path(config.get(image_key))
        ...
        try:
            ...
            if config.get("has_cobb_option") and getattr(self, "show_cobb_var", ctk.BooleanVar(value=False)).get():
                cobb_path = resource_path(config.get("cobb_overlay"))
            ...
            # Wrap the font path
            font_path = resource_path(config["font_file"])
            try:
    ...
    def _update_editor_preview(self, *args):
        img = self._generate_plate_image()
        if img:
    ...
    def _send_to_map_maker(self):
        ...
        self.mm_drop_zone.update_preview(temp_path)
        self.mm_drop_zone.configure(border_color=COLORS["accent_success"])

        # Load the preview and switch tabs
        self._load_preview_image(temp_path)
        self.show_page("map_maker")
        
        # Force a refresh after the tab animation finishes
        # This re-links the sliders to the new thumbnail image
        self.after(350, self._schedule_preview_update)

    def _send_map_to_compiler(self):
        source_path = self.mm_drop_zone.get_path()
        ...
        if not hasattr(self, 'last_mm_map') or not os.path.exists(self.last_mm_map):
            ...
            final_map = self._create_normal_map_data(img, self.base_intensity.get(), self.base_blur.get(), self.base_extrude.get())
    ...
    def setup_presets_page(self):
        ...
        ctk.CTkOptionMenu(
            ...
            corner_radius=6, command=lambda _: self.save_config(silent=True)
        ).pack(side="left", padx=(0, 20))
        ...
        self.preset_dir_entry.bind("<Button-1>", lambda e: self._browse_gen_output_dir())
    ...
    def refresh_presets(self, force=False):
        ...
        for item in self.preset_data:
            ...
            if img_path in self.image_cache:
                ...
                threading.Thread(target=self._load_preset_preview, args=(img_path, img_label, item['region']), daemon=True).start()
            else:
                img_label.configure(image=self.load_icon("image.png", size=32), text="")
            ...
            ctk.CTkButton(card, text=btn_text, state=btn_state, fg_color=btn_color, command=lambda i=item: self.toggle_preset_cart(i)).pack(pady=(0, 20), padx=20, fill="x")
    ...
    def _load_preset_preview(self, path, label, region):
        def _background_task():
            try:
                ...
                self.ui_queue.put(lambda: self._finalize_preset_ui(pil_img, path, label))
    ...
    def _finalize_preset_ui(self, pil_img, path, label):
        try:
    ...
    def _handle_preset_error(self, label):
        try:
            if label.winfo_exists():
                error_icon = self.load_icon("image.png", size=32)
    ...
    def toggle_preset_cart(self, item):
        region_key = 'us' if item['region'] == "US & MX" else 'eu'
        ...
        def process():
            try:
                ...
                if self.preset_cart['eu']: 
                    self.region_var.set("EU & UK")
                    self._process_files(self.preset_cart['eu']['img'], self.preset_cart['eu']['nrml'], out_dir, silent=True)
                    # Add to history
                    self.history.append({
                        "region": "EU & UK", 
                ...
                if self.preset_cart['us']: 
                    self.region_var.set("US & MX")
                    self._process_files(self.preset_cart['us']['img'], self.preset_cart['us']['nrml'], out_dir, silent=True)
                    # Add to history
                    self.history.append({
                        "region": "US & MX", 
    ...
    def setup_dashboard_page(self):
        ...
        header = ctk.CTkLabel(
            self.dashboard_page, 
            text="Welcome back! ", 
            image=self.load_icon("hello.png", size=32),
            compound="right",
        ...
        self.dash_silent_switch = ctk.CTkSwitch(
            ...
            button_color=COLORS["accent_primary"], command=lambda: self.save_config(silent=True)
        )
    ...
    def refresh_dashboard(self):
        ...
        for path, label in checks:
            ...
            if label == self.health_sz_status and not exists:
                exists = os.path.exists(resource_path("7za.exe"))
        ...
        threading.Thread(target=self._load_active_plates, daemon=True).start()
    ...
    def load_last_project(self):
        ...
        if img and os.path.exists(img):
            ...
            self.image_drop_zone.update_preview(img)
            self.image_drop_zone.configure(border_color=COLORS["accent_success"])
        ...
        if nrml and os.path.exists(nrml):
            ...
            self.nrml_drop_zone.update_preview(nrml)
            self.nrml_drop_zone.configure(border_color=COLORS["accent_success"])
            
        # Swap to the compiler page
        self.show_page("compiler")

    def prompt_clear_backups(self):
        ...
        if messagebox.askyesno("Confirm", "This will search the 'plates' folder inside this zip and delete ALL backup (.bak) files.\n\nContinue?"):
            threading.Thread(target=self._process_clear_backups, args=(target_zip,), daemon=True).start()

    def _process_clear_backups(self, target_zip):
        try:
            sz_path = self.sz_path_var.get().strip('"')
    ...
    def _load_active_plates(self):
        import zipfile
        try:
            out_path = getattr(self, "gen_output_dir_var", ctk.StringVar()).get()
            if out_path == "Not Selected" or not out_path:
                self._set_active_plate_ui("eu", None, "No Output Selected")
                self._set_active_plate_ui("us", None, "No Output Selected")
                return
            ...
            if not os.path.exists(target_zip) or not os.path.isfile(target_zip):
                self._set_active_plate_ui("eu", None, "Textures.zip not found")
                self._set_active_plate_ui("us", None, "Textures.zip not found")
                return
            ...
            self._set_active_plate_ui("eu", eu_img, "Default EU Plate")
            self._set_active_plate_ui("us", us_img, "Default US Plate")
            
        except zipfile.BadZipFile:
            self._set_active_plate_ui("eu", None, "Invalid Textures.zip")
            self._set_active_plate_ui("us", None, "Invalid Textures.zip")
        except Exception as e:
            self._set_active_plate_ui("eu", None, "Error Reading Plates")
            self._set_active_plate_ui("us", None, "Error Reading Plates")
            
    def _set_active_plate_ui(self, region, img, fallback_text):
    ...
    def animate_status_dot(self):
        ...
        try:
            if self.status_dot.winfo_exists():
                ...
                self.after(50, self.animate_status_dot)
        except Exception:
            pass

    def open_mask_painter(self, edit=False):
        source = self.mm_drop_zone.get_path()
        ...
        mask = None
        if edit:
            mask = self.mm_mask_drop_zone.get_path()
    ...
    def open_normal_painter(self):
        ...
        def _prepare():
            try:
                ...
                self.after(0, lambda: NormalPainter(self, final_img, self._save_painted_normal_map))
            except Exception as e:
    ...
    def _save_painted_normal_map(self, final_img, send_to_compiler=False):
